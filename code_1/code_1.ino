...
AudioConnection          patchCord101(bitcrusher2, 0, effectmixb4, 1);
AudioConnection          patchCord102(delay1, 0, delaymixa1, 0);
AudioConnection          patchCord103(delay1, 1, delaymixa1, 1);
AudioConnection          patchCord104(delay1, 2, delaymixa1, 2);
AudioConnection          patchCord105(delay1, 3, delaymixa1, 3);
AudioConnection          patchCord106(delay1, 4, delaymixa2, 0);
AudioConnection          patchCord107(delay1, 5, delaymixa2, 1);
...
// start note
void startNote(int voice, int values[]) {
  sines[i].frequency(returnFrequency[values[0]]);
  sines[i].amplitude(values[1]*scale127);
  envelopes[i].noteOn();
}
...
// mod effect values
void addeffectvalues() {
  if (digitalRead(sbutton[0]) == 1) {
    if (effectvol < 1) {
      effectvol = effectvol + 0.1;
    }
  }
  if (digitalRead(sbutton[1]) == 1) {
    if (effectvol > 0) {
      effectvol = effectvol - 0.1;
    }
  }
  if (digitalRead(sbutton[2]) == 1) {
    if (effectvolcontrol < 5) {
      effectvolcontrol = effectvolcontrol + 1;
    }
  }
  if (digitalRead(sbutton[3]) == 1) {
    if (effectvolcontrol > 0) {
      effectvolcontrol = effectvolcontrol - 1;
    }
  }
  if (digitalRead(sbutton[4]) == 1) {
    if (chorusvoices < 6) {
      chorusvoices = chorusvoices + 1;
    }
  }
  if (digitalRead(sbutton[5]) == 1) {
    if (chorusvoices > 0) {
      chorusvoices = chorusvoices - 1;
    }
  }
  if (digitalRead(sbutton[6]) == 1) {
    if (flangeoffsetnum < 8) {
      flangeoffsetnum = flangeoffsetnum + 1;
    }
  }
  if (digitalRead(sbutton[7]) == 1) {
    if (flangeoffsetnum > 0) {
      flangeoffsetnum = flangeoffsetnum - 1;
    }
  }
  if (digitalRead(sbutton[8]) == 1) {
    if (flangedepthnum < 8) {
      flangedepthnum = flangedepthnum + 1;
    }
  }
  if (digitalRead(sbutton[9]) == 1) {
    if (flangedepthnum > 0) {
      flangedepthnum = flangedepthnum - 1;
    }
  }
  if (digitalRead(sbutton[10]) == 1) {
    if (flangefreq < 1.0) {
      flangefreq = flangefreq + 0.1;
    }
  }
  if (digitalRead(sbutton[11]) == 1) {
    if (flangefreq > 0) {
      flangefreq = flangefreq - 0.1;
    }
  }
  if (digitalRead(sbutton[12]) == 1) {
    if (reverbamount < 1) {
      reverbamount = reverbamount + 0.1;
    }
  }
  if (digitalRead(sbutton[13]) == 1) {
    if (reverbamount > 0) {
      reverbamount = reverbamount - 0.1;
    }
  }
  if (digitalRead(sbutton[14]) == 1) {
    if (delaychannels < 8) {
      delaychannels = delaychannels + 1;
    }
  }
  if (digitalRead(sbutton[15]) == 1) {
    if (delaychannels > 0) {
      delaychannels = delaychannels - 1;
    }
  }
  if (digitalRead(sbutton[16]) == 1) {
    if (bitbits < 16) {
      bitbits = bitbits + 1;
    }
  }
  if (digitalRead(sbutton[17]) == 1) {
    if (bitbits > 1) {
      bitbits = bitbits - 1;
    }
  }
  if (digitalRead(sbutton[18]) == 1) {
    if (granularspeed < 8.0) {
      granularspeed = granularspeed + 0.25;
    }
  }
  if (digitalRead(sbutton[19]) == 1) {
    if (granularspeed > 0.25) {
      granularspeed = granularspeed - 0.25;
    }
  }
...
// flanger
void modflanger() {
  if (flageoffset != flangeoffsetcomp || flangedepth != flangedepthcomp || flangefreq != flangefreqcomp) {
    flange1.voices(flangeoffset, flangedepth, flangefreq);
    flange2.voices(flangeoffset, flangedepth, flangefreq);
    flangeoffsetcomp = flageoffset;
    flangedepth = flangedepthcomp;
    flangefreq = flangefreqcomp;
  }
}
